        ...
        with StereoCamera(self.config) as cam:
            while self.running:
                ...
                for det in detections_l:
                    cv2.circle(overlay_l, (int(det.pos[0] / 2), int(det.pos[1] / 2)), int(det.size / 4), 255, 1)
                for det in detections_r:
                    cv2.circle(overlay_r, (int(det.pos[0] / 2), int(det.pos[1] / 2)), int(det.size / 4), 255, 1)
                ...
                if detections_3d and detections_3d[0].confidence_01 >= conf_thresh:
                    ...
                    for img, pos in [(overlay_l, detection.pos_2d_raw_l), (overlay_r, detection.pos_2d_raw_r)]:
                        ...
                        line_length = 1000
                        cv2.line(img, (x - line_length, y), (x + line_length, y), 255, 1)
                        cv2.line(img, (x, y - line_length), (x, y + line_length), 255, 1)
