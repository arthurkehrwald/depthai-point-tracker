...
def show_preview(img: cv2.typing.MatLike, name: str, found_centroid: bool, centroid_x: float, centroid_y: float):
    if found_centroid:
        cx = int(centroid_x)
        cy = int(centroid_y)
        circle_pos = (cx, cy)
        ...
        while pipeline.isRunning():
            ...
            if success_l and success_r and img_l is not None and img_r is not None:
                success_l, cX_l, cY_l = try_get_centroid(
                    img_l, threshold01=threshold01
                )
                success_r, cX_r, cY_r = try_get_centroid(
                    img_r, threshold01=threshold01
                )
                show_preview(img_l, "Point Tracker", success_l, cX_l, cY_l)
